...
Registers the `my_dsh_plugin` tool via `defineTool()`.
...
dsh plugin --profile my-profile add ./my-dsh-plugin
dsh --profile my-profile       # watch for: [my-dsh-plugin] registered "my_dsh_plugin" — listed=true
```
...
```sh
dsh --profile my-profile --dump-config | grep my-dsh-plugin   # 配置层含本行
